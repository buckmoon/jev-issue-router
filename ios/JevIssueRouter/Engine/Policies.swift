...
/// Port of `issue_router/policies.py`. The English objectives are what Jev receives; the Japanese
/// descriptions are shown in the picker, exactly as in the macOS app.
struct PolicyGuidance {
    let recommendationScope: String
    let costBasis: String?
    let summary: String
    let nextStep: String
    let reevaluateWhen: String
    let reevaluationPolicy: String
    let automaticEscalation: Bool

    var json: JSONValue {
        var pairs: [(String, JSONValue)] = [("recommendation_scope", .string(recommendationScope))]
        if let costBasis { pairs.append(("cost_basis", .string(costBasis))) }
        pairs += [("summary", .string(summary)), ("next_step", .string(nextStep)),
                  ("reevaluate_when", .string(reevaluateWhen)),
                  ("reevaluation_policy", .string(reevaluationPolicy)),
                  ("automatic_escalation", .bool(automaticEscalation))]
        return .fields(pairs)
    }
}

enum Policies {
    /// Picker order matches the Python dictionary order.
    static let names = ["balanced", "quality", "cost", "value", "total-cost", "min-cost", "max-quality"]

    static let objectives: [String: String] = [
    ...
    static let guidance: [String: PolicyGuidance] = [
        "value": PolicyGuidance(
            recommendationScope: "bounded_first_attempt", costBasis: "qualitative_not_measured",
            summary: "初回試行のコスパを優先する候補です。",
            nextStep: "小さく検証できる範囲で試し、得られた進捗と検証結果を確認してください。",
            reevaluateWhen: "進捗が得られない、同じ失敗を繰り返す、または検証で重大な問題が出る場合。",
            reevaluationPolicy: "total-cost", automaticEscalation: false),
        "min-cost": PolicyGuidance(
            recommendationScope: "cheapest_usable_attempt", costBasis: "qualitative_not_measured",
            summary: "コスト最優先の候補です。初回での完遂や品質は優先していません。",
            nextStep: "結果を必ず検証し、やり直しや人手の修正が必要になる前提で使ってください。",
            reevaluateWhen: "使える進捗が得られない、または修正の手間がモデル費用の節約を上回る場合。",
            reevaluationPolicy: "total-cost", automaticEscalation: false),
        "max-quality": PolicyGuidance(
            recommendationScope: "design_quality", costBasis: nil,
            summary: "設計品質最優先の候補です。コストと所要時間は考慮していません。",
            nextStep: "実装前に設計方針（構成、データ設計、影響範囲）を出させ、人がレビューしてから進めてください。",
            reevaluateWhen: "設計判断の少ない作業だと分かった、または費用・時間の制約が出てきた場合。",
            reevaluationPolicy: "total-cost", automaticEscalation: false),
        "total-cost": PolicyGuidance(
            recommendationScope: "verified_completion", costBasis: "qualitative_not_measured",
            summary: "再試行・レビュー・手戻り時間を含めた総コストを優先する候補です。",
            nextStep: "モデル利用量だけでなく、検証・レビュー・手戻りにかかった時間も確認してください。",
            reevaluateWhen: "想定より再試行や人手の修正が増えた、または費用・時間の制約が変わった場合。",
            reevaluationPolicy: "total-cost", automaticEscalation: false),
    ]
}
