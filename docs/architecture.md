...
- **初回の登録**: `model_watch.json` に `"seeded": true` が無い状態で `--write` を実行すると、新規モデルの提案の代わりに、
  現在の一覧にあるカタログ外のモデルをすべて `ignored` に登録します（カタログは変更しません）。
  各社の過去のモデルがまとめて提案されるのを防ぐためです。登録内容はPRで確認し、採用すべきものがあれば `ignored` から外してください。
  3社すべての一覧を取得できたときだけ実行します。
